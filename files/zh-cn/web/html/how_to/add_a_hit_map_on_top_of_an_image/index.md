...
若在 {{htmlelement('a')}} 标签嵌套图像，整个图像链接到一个地址。图像映射则有多个活动区域（称为“热点”），可以链接到不同的资源地址。

图像映射曾是很常用的导航方式，但前提是考虑性能与无障碍。

> [!WARNING]
> 多个图像引用同一图像映射可能引起浏览器行为异常，严重影响可用性和可访问性。例如，当用户在 Safari 和基于 Chromium 的浏览器用键盘导航包含重复图像映射的图像时，后者使用同一图像地图的图像实例会被完全跳过。在 Firefox 中，所有图像地图会同时获得键盘焦点，当用户通过键盘导航跳过该图像时，下一个获得焦点的元素将是最后一个图像实例之后的元素， 跳过两个图像之间的所有内容。

[文字链接](/zh-CN/docs/Learn_web_development/Core/Structuring_content/Creating_links)（可能辅以 CSS）比图像映射更具优势：文本链接更轻量级、好维护、更易于 SEO，而且支持更多形式需求的访问（如，屏幕阅读器、纯文本浏览器、翻译服务等）。
